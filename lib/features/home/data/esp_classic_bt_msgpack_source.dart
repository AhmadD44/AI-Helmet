...
class EspBtClassicSource {
  ...
  void _onBytes(Uint8List bytes) {
    ...
    try {
      ...
      if (debugLog) {
        print(
          "📥 RX (${incomingStr.length} chars): ${_formatIncomingData(incomingStr)}",
        );
      }
    ...
    while (_messageBuffer.isNotEmpty) {
      ...
      if (!_messageBuffer.startsWith('{')) {
        ...
        if (nextBrace != -1) {
          if (debugLog) {
            print(
              "⚠️ Found { at position $nextBrace, removing ${nextBrace} chars of garbage",
            );
            print(
              "⚠️ Removing: ${_messageBuffer.substring(0, math.min(nextBrace, 100))}",
            );
          }
          ...
          if (debugLog) {
            print(
              "✅ Buffer cleaned, new start: ${_messageBuffer.substring(0, math.min(50, _messageBuffer.length))}",
            );
          }
        } else {
        ...
        if (debugLog) {
          ...
          if (_messageBuffer.contains('{')) {
            ...
            if (_messageBuffer.length > 5000) {
              print(
                "⚠️⚠️⚠️ BUFFER TOO LARGE (${_messageBuffer.length} chars)!",
              );
              ...
              if (nextTelemetry != -1) {
                ...
                int braceBeforeTelemetry = _messageBuffer.lastIndexOf(
                  '{',
                  nextTelemetry,
                );
                if (braceBeforeTelemetry != -1) {
                  print(
                    "⚠️ Attempting emergency recovery at position $braceBeforeTelemetry",
                  );
                  _messageBuffer = _messageBuffer.substring(
                    braceBeforeTelemetry,
                  );
  ...
  String _fixEsp32Json(String jsonStr) {
    ...
    if (fixed.contains('"velocity":{"kmh"') &&
        !fixed.contains('"velocity":{"kmh":')) {
      // Add missing colon and value
      fixed = fixed.replaceAll('"velocity":{"kmh"', '"velocity":{"kmh":0');
    }

    if (fixed.contains('"velocity":{"kmh":') &&
        !fixed.contains('"velocity":{"kmh":0}')) {
  ...
  void _trySalvageJson(String jsonStr) {
    ...
    if (firstBrace != -1 && lastBrace != -1 && lastBrace > firstBrace) {
      ...
      if (potentialJson.contains('"type":"telemetry"') &&
        ...
        try {
          final Map<String, dynamic> parsed = _parseJsonWithAutoConversion(
            potentialJson,
          );
